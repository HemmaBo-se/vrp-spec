...
VRP may compose with adjacent agent-commerce standards. UCP can handle checkout and order lifecycle, including lodging flows as the UCP lodging profile matures; AP2 can handle payment mandates; x402 can carry inline agent-native settlement as an optional, additive payment rail (see §5.2); MCP can expose future tool bindings; and A2A can carry future agent-to-agent negotiation. VRP v0.1 does not define those runtime flows.
...
### 5.2 Optional payment rails (x402 binding)

`direct_booking_url` (§5.1) is the only REQUIRED booking action and routes the
guest to the host's own booking flow, settled by the host's own payment
processor (fiat, e.g. Stripe). That is the default for v0.1.

A node MAY additionally advertise agent-native payment rails in the optional
`booking.payment_options[]` array. Each entry names a `rail` (e.g. `x402`) and a
`pay_to` payee the agent pays directly — for example the host's OWN x402 wallet,
with the same sovereignty as the host's own fiat processor. This lets an
x402-speaking agent settle inline in the same request without leaving the flow,
while fiat via `direct_booking_url` stays the default a human guest uses.

The binding is OPTIONAL and ADDITIVE:

- Absence of `payment_options` means fiat-via-`direct_booking_url` only. Most
  nodes will not set it in v0.1.
- `payment_options` is a signed field inside the JWS (§5), so listing a rail is
  a signed claim the node can settle AND confirm a booking on it. A node MUST
  NOT list a rail it cannot honor; a listed-but-unhonored rail is a false offer.
- `payment_recipient` stays `host` on every rail. A rail whose `pay_to` is an
  intermediary, an OTA, or a HemmaBo-operated payee MUST NOT be listed.
- Rails MUST NOT gate discovery, verification, or offer retrieval. Charging an
  agent per request to find or verify a node contradicts the no-gatekeeper model
  (§1) and is out of scope.

Rail-specific settlement, refunds/chargebacks, and reconciling an off-band
payment with the booking are runtime concerns VRP v0.1 does not define (as with
UCP, ACP, and AP2). In particular, x402's stablecoin settlement and
no-chargeback model MUST be reconciled with vacation-rental cancellation and
refund semantics before a node turns a rail on live; until then a node SHOULD
omit `payment_options` and rely on `direct_booking_url`.
