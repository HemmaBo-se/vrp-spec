...
from the cache bound below, so the cache bound governs revocation latency alone.

**Revocation.** Removing a key from the JWKS revokes it: offers signed by a removed
key no longer verify. Offer freshness (`valid_until`, §6) does NOT bound this — an
attacker holding a leaked private key can mint fresh offers with a future
`valid_until` that pass the freshness check. The only thing that stops a conforming
verifier from honoring such offers is the key being absent from the JWKS it holds.
Therefore:

- A verifier MUST NOT cache a node's JWKS for longer than **5 minutes**, and SHOULD
  honor a shorter `Cache-Control` `max-age` served by the node. This bound is the
  revocation latency: a compromised key is fully de-trusted by conforming verifiers
  within the cache window after it is removed from the JWKS.
